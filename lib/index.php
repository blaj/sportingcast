...
abstract class _Model
{
     ...
     * @param array $credentials Database credentials: dsn - data source name, user - database username, password - database password
     */
    static function setCredentials($credentials=array())
    {
    ...
    {
        if (empty(self::$db) && !empty(self::$credentials['dsn']))
        {
            self::$db = new PDO(self::$credentials['dsn'], self::$credentials['user'], self::$credentials['password']);
